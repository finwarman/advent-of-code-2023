...
def array_flip_fix_smudge(grid, cols):
    ...
    for i in range(1, cols):
        ...
        if np.sum(left_segment != right_segment) == 1:
            return i
    return None
...
for grid in grids:
    ...
    h_count = array_flip_fix_smudge(grid, cols)
    if h_count is not None:
        col_count += h_count
        continue

    v_count = array_flip_fix_smudge(grid.T, rows)
